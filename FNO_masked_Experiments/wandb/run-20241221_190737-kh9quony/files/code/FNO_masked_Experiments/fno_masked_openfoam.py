...
###DEBUG##########################################
# Extract mask, velocity, and coordinates for debugging
mask_debug = data_tensor[..., -1].cpu().numpy()  # Extract mask (last channel)
horizontal_velocity = data_tensor[..., 0].cpu().numpy()  # Horizontal velocity
vertical_velocity = data_tensor[..., 1].cpu().numpy()  # Vertical velocity
x_coords_debug = data_tensor[..., 2].cpu().numpy()  # X coordinates
y_coords_debug = data_tensor[..., 3].cpu().numpy()  # Y coordinates

# Find indices where mask == 0
hole_indices = np.where(mask_debug == 0)

# Debug outputs
print("Number of hole points (mask == 0):", len(hole_indices[0]))
print("Horizontal velocity at hole centers:")
print(horizontal_velocity[hole_indices])
print("Vertical velocity at hole centers:")
print(vertical_velocity[hole_indices])
print("X coordinates at hole centers:")
print(x_coords_debug[hole_indices])
print("Y coordinates at hole centers:")
print(y_coords_debug[hole_indices])
...
wandb.init(
    ...
    name=f"FNO_masked_{dataset_name}_{200}",  # Optional, gives each run a unique name
