...
class FNO3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width):
        ...
        self.fc2 = nn.Linear(128, 1)  # Output 33 features (3 channels * 11 timesteps)
...
output_folder = "NS-Sines-Poseidon-2-trial-u"
...
modes = 12
width = 32
out_dim = 1 #4, changed to 3 as we have 3 channels.
...
x_train = velocity[:ntrain, :10, :1, :, :] #Input first 10 time steps for training
print("Shape of xtrain:", x_train.shape)
y_train = velocity[:ntrain, 10:20, :1, :, :] #Output all remaining time steps for training
print("Shape of ytrain:", y_train.shape)

x_test = velocity[-ntest:, :10, :1, :, :] #input first 10 time steps for testing
y_test = velocity[-ntest:, 10:20, :1, :, :] #output remanining time steps for testing
...
wandb.init(
    ...
    name=f"{dataset_name}_{1}_trial_u",  # Optional, gives each run a unique name
...
for ep in range(epochs):
    ...
    if ep % 50 == 0:
        ...
        for ch in range(1):  # Loop over channels (0 for horizontal, 1 for vertical)
            ...
            for i, t in enumerate(time_indices):
                ...
                im1 = ax[2 * ch, i].imshow(truth[:, :, t], cmap='gist_ncar', vmin=vmin, vmax=vmax)
                ...
                im2 = ax[2 * ch + 1, i].imshow(pred[:, :, t], cmap='gist_ncar', vmin=pred_min, vmax=pred_max)
