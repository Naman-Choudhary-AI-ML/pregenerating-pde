...
class FNO3d(nn.Module):
    ...
    def forward(self, x):
        ...
        print("X shape at start of FNO forward:",x.shape)
        x = torch.cat((x, grid), dim=-1)
        print("X shape after concat:",x.shape)
        x = self.fc0(x)
        x = x.permute(0, 4, 1, 2, 3)
        print("X shape after fc0 and permuting:",x.shape)
        ...
        print("X shape before fc1 after all spectral and conv and permute:",x.shape)
        x = self.fc1(x)
        x = F.gelu(x)
        x = self.fc2(x)
...
# configs
################################################################
DATA_PATH = '/home/namancho/datasets/plasticity/plas_N987_T20.mat'
...
wandb.init(
    ...
    name=f"plasticity_{1}_baseline",  # Optional, gives each run a unique name
...
for ep in range(epochs):
    ...
    if ep%50==0:
        ...
        # fig.show()
        output_folder = './plasticity_baseline'
