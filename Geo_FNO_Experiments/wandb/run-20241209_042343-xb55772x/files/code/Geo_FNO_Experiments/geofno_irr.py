...
# configs
################################################################
DATA_PATH = '/home/namancho/datasets/NS-Gauss-Irr-Openfoam/openfoam.npy'
...
x_train = data[:ntrain, :10, :, :, :] #Input first 10 time steps for training
print("Shape of xtrain:", x_train.shape)
y_train = data[:ntrain, 10:20, :, :, :] #Output all remaining time steps for training
print("Shape of ytrain:", y_train.shape)

x_test = data[-ntest:, :10, :, :, :] #input first 10 time steps for testing
y_test = data[-ntest:, 10:20, :, :, :] #output remanining time steps for testing
...
for ep in range(epochs):
    ...
    if ep % 50 == 0:
        ...
        vmin_u = np.nanmin(truth_masked[:, :, :, 0])
        vmax_u = np.nanmax(truth_masked[:, :, :, 0])
        vmin_v = np.nanmin(truth_masked[:, :, :, 1])
        vmax_v = np.nanmax(truth_masked[:, :, :, 1])

        pred_vmin_u = np.nanmin(pred_masked[:, :, :, 0])
        pred_vmax_u = np.nanmax(pred_masked[:, :, :, 0])
        pred_vmin_v = np.nanmin(pred_masked[:, :, :, 1])
        pred_vmax_v = np.nanmax(pred_masked[:, :, :, 1])
        ...
        for ch in range(2):  # Loop over channels (0 for horizontal, 1 for vertical)
            ...
            pred_min = pred_vmin_u if ch == 0 else pred_vmin_v
            pred_max = pred_vmax_u if ch == 0 else pred_vmax_v

            for i, t in enumerate(time_indices):
                # Plot truth for current channel
                im1 = ax[2 * ch, i].imshow(truth_masked[:, :, t, ch], cmap='gist_ncar', vmin=vmin, vmax=vmax)
                ...
                im2 = ax[2 * ch + 1, i].imshow(pred_masked[:, :, t, ch], cmap='gist_ncar', vmin=pred_min, vmax=pred_max)
