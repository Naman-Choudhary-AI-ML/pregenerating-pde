...
for epoch in range(epochs):
    ...
    for step, (input_batch, output_batch) in enumerate(train_loader):
        input_batch, output_batch = input_batch.cuda(), output_batch.cuda()
    ...
    if epoch % freq_plot == 0 or epoch == epochs - 1:
        ...
        num_fields = input_sample.shape[0]  # e.g., 4 for density, pressure, velocity_x, velocity_y

        # Create a figure with num_fields rows and 3 columns
        fig, axes = plt.subplots(num_fields, 3, figsize=(12, 4 * num_fields))

        for field in range(num_fields):
            # Determine color limits for consistent scaling across Ground Truth and Prediction
            vmin = min(np.min(output_sample[field]), np.min(pred_sample[field]))
            vmax = max(np.max(output_sample[field]), np.max(pred_sample[field]))

            # Plot Input
            im0 = axes[field, 0].imshow(
                input_sample[field], cmap="RdBu_r", interpolation="none"
            )
            axes[field, 0].set_title(f"Input (Field {field + 1})")
            axes[field, 0].set_xticks([])
            axes[field, 0].set_yticks([])
            plt.colorbar(im0, ax=axes[field, 0], fraction=0.046, pad=0.04)

            # Plot Ground Truth
            im1 = axes[field, 1].imshow(
                output_sample[field], cmap="RdBu_r", interpolation="none", vmin=vmin, vmax=vmax
            )
            axes[field, 1].set_title(f"Ground Truth (Field {field + 1})")
            axes[field, 1].set_xticks([])
            axes[field, 1].set_yticks([])
            plt.colorbar(im1, ax=axes[field, 1], fraction=0.046, pad=0.04)

            # Plot Prediction
            im2 = axes[field, 2].imshow(
                pred_sample[field], cmap="RdBu_r", interpolation="none", vmin=vmin, vmax=vmax
            )
            axes[field, 2].set_title(f"Prediction (Field {field + 1})")
            axes[field, 2].set_xticks([])
            axes[field, 2].set_yticks([])
            plt.colorbar(im2, ax=axes[field, 2], fraction=0.046, pad=0.04)
