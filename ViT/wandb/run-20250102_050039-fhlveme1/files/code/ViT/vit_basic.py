...
for epoch in range(epochs):
    ...
    if epoch % freq_plot == 0 or epoch == epochs - 1:
        ...
        for field in range(num_fields):
            # Determine color limits for consistent scaling across Input, Ground Truth, and Prediction
            vmin = min(np.min(input_sample[field]), np.min(output_sample[field]), np.min(pred_sample[field]))
            vmax = max(np.max(input_sample[field]), np.max(output_sample[field]), np.max(pred_sample[field]))

            # Plot Input
            im0 = axes[field, 0].imshow(
                input_sample[field], cmap="RdBu_r", interpolation="none", vmin=vmin, vmax=vmax
            )
            ...
            axes[field, 1].set_title(f"Output Ground Truth ({field_names[field]})")
            ...
            # Add a single horizontal color bar below all three plots for the current field
            cbar = fig.colorbar(
                im2, ax=axes[field, :], orientation="horizontal", fraction=0.05, pad=0.1
            )
            cbar.set_label(f"{field_names[field]} Color Scale")
