...
output_dir = "NS_SL_dataset"  # Directory to store dataset
base_case_dir = "./"       # Base OpenFOAM case folder
channels = 2
final_output_dir = "/home/namancho/datasets/NS-SL-Openfoam"  # Combined reshaped dataset output file
dataset_dir = "./NS_SL_dataset"
...
def generate_shear_layer_velocity(nx, ny, max_velocity=2.421025):
    """
    Generate velocity fields for NS-SL (Shear Layer) with robust handling for potential `nan` values.
    
    Parameters:
        nx, ny: int - Grid dimensions (number of points in x and y directions).
        max_velocity: float - Maximum allowable velocity magnitude for scaling.
    ...
    p = np.random.randint(7, 13)  # Randomly choose p between 7 and 12
    delta = 0.025
    rho = np.random.uniform(0.08, 0.12)  # Thickness parameter
    xi = np.random.uniform(-0.0625, 0.0625)  # Perturbation shift
    alpha = np.random.uniform(0, 1, p)
    beta = np.random.uniform(0, 2 * np.pi, p)

    # Generate σδ(x) (Perturbation Term)
    sigma_delta = xi + delta * sum(
        alpha[k] * np.sin(2 * np.pi * k * X - beta[k]) for k in range(p)
    )

    # Horizontal velocity (u): Shear layer with perturbations
    u_x = np.where(
        Y + sigma_delta <= 0.5,
        np.tanh(2 * np.pi * (Y - 0.25) / rho),
        np.tanh(2 * np.pi * (0.75 - Y) / rho),
    )

    # Vertical velocity (v): Initially zero
    u_y = np.zeros_like(X)

    # Handle edge cases dynamically
    if np.std(u_x) == 0:  # Standard deviation of `u_x` is zero
        u_x += np.random.normal(scale=1e-3, size=u_x.shape)  # Add small random noise
    if np.std(u_y) == 0:  # Standard deviation of `u_y` is zero
        u_y += np.random.normal(scale=1e-3, size=u_y.shape)  # Add small random noise

    # Normalize Horizontal and Vertical Velocities to Match Target Statistics
    target_mean_std = [
        (-7.253363e-12, 0.874058),  # Horizontal Velocity (u)
        (-1.508875e-11, 0.320131),  # Vertical Velocity (v)
    ]

    for i, (data, (target_mean, target_std)) in enumerate(zip([u_x, u_y], target_mean_std)):
        if np.std(data) == 0:  # Safeguard against division by zero during normalization
            data += np.random.normal(scale=1e-3, size=data.shape)  # Add small random noise
...
for traj in range(num_trajectories):
    ...
    velocity = generate_shear_layer_velocity(nx, ny)
