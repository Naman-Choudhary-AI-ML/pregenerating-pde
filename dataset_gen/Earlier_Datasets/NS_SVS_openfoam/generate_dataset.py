...
output_dir = "NS_SVS_dataset"  # Directory to store dataset
base_case_dir = "./"       # Base OpenFOAM case folder
channels = 2
final_output_dir = "/home/namancho/datasets/NS-SVS-Openfoam"  # Combined reshaped dataset output file
dataset_dir = "./NS_SVS_dataset"
...
def generate_sinusoidal_vortex_sheet_velocity(nx, ny, max_velocity=0.493763):
    """
    Generate velocity fields for NS-SVS (Sinusoidal Vortex Sheet).
    ...
    # --- Equation (50): Gamma ---
    p = 10  # Number of sinusoidal terms
    alpha = np.random.uniform(0, 0.003125, p)  # Amplitudes
    beta = np.random.uniform(0, 1, p)  # Phases
    Gamma = (
        0.5
        + 0.2 * np.sin(2 * np.pi * X)
        + sum(alpha[i] * np.sin(2 * np.pi * (X + beta[i])) for i in range(p))
    )

    # --- Equation (47): ω_0(x) ---
    omega = np.zeros_like(X)
    rho = 5 / 128  # From the paper

    # Apply smoothing function ψ(r) to calculate ωρ0
    for i in range(nx):
        for j in range(ny):
            distance = np.abs(Y[i, j] - Gamma[i, j])  # Distance to Gamma
            omega[i, j] = rho ** -2 * smoothing_function(distance / rho)  # Equation (48)

    # Subtract the integral term to enforce zero-mean vorticity
    omega -= np.mean(omega)  # Implements \(- \int_{T^2} d\Gamma\) in Equation (47)

    # --- Solve for stream function ψ (Poisson equation: ∇²ψ = ω) ---
    ...
    # Handle potential zero standard deviation issues
    if np.std(u_x) == 0:
        u_x += np.random.normal(scale=1e-3, size=u_x.shape)
    if np.std(u_y) == 0:
        u_y += np.random.normal(scale=1e-3, size=u_y.shape)

    # --- Normalize to Target Statistics ---
    target_mean_std = [
        (-1.512145e-13, 0.114872),  # Horizontal Velocity (u)
        (-1.205742e-11, 0.094127),  # Vertical Velocity (v)
    ]
...
def smoothing_function(r):
    """
    Smoothing function ψ(r) as per Equation (49), scaled by ρ^-2 from Equation (48).
    """
    factor = 80 / (7 * np.pi)  # Scaling factor for ψ
    r_plus = np.maximum(r, 0)  # Ensure r is non-negative
    return factor * (
        (r_plus + 1) ** 3
        - 4 * (r_plus + 0.5) ** 3
        + 6 * r_plus ** 3
...
for traj in range(num_trajectories):
    ...
    velocity = generate_sinusoidal_vortex_sheet_velocity(nx, ny)
