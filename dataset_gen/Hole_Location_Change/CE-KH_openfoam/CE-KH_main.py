...
def get_blocks_around_hole(N, i_hole_min, i_hole_max, j_hole_min, j_hole_max):
    """
    Dynamically generate the 8 blocks around a rectangular hole in an N x N domain.
    Returns a list of (iRange, jRange).
    """
    blocks = []

    # Bottom-left
    if i_hole_min > 0 and j_hole_min > 0:
        blocks.append((range(0, i_hole_min), range(0, j_hole_min)))

    # Bottom-center
    if j_hole_min > 0:
        blocks.append((range(i_hole_min, i_hole_max + 1), range(0, j_hole_min)))

    # Bottom-right
    if i_hole_max < N - 1 and j_hole_min > 0:
        blocks.append((range(i_hole_max + 1, N), range(0, j_hole_min)))

    # Middle-left
    if i_hole_min > 0:
        blocks.append((range(0, i_hole_min), range(j_hole_min, j_hole_max + 1)))

    # Middle-right
    if i_hole_max < N - 1:
        blocks.append((range(i_hole_max + 1, N), range(j_hole_min, j_hole_max + 1)))

    # Top-left
    if i_hole_min > 0 and j_hole_max < N - 1:
        blocks.append((range(0, i_hole_min), range(j_hole_max + 1, N)))

    # Top-center
    if j_hole_max < N - 1:
        blocks.append((range(i_hole_min, i_hole_max + 1), range(j_hole_max + 1, N)))

    # Top-right
    if i_hole_max < N - 1 and j_hole_max < N - 1:
        blocks.append((range(i_hole_max + 1, N), range(j_hole_max + 1, N)))
    
    x_hole_min = i_hole_min / N
    x_hole_max = (i_hole_max + 1) / N  # +1 to include upper edge
    y_hole_min = j_hole_min / N
    y_hole_max = (j_hole_max + 1) / N

    logging.info(f"Corresponding hole coordinates: "
                 f"x_hole=({x_hole_min:.3f}, {x_hole_max:.3f}), "
                 f"y_hole=({y_hole_min:.3f}, {y_hole_max:.3f})")
...
def generate_U_file(folder, cx, cy, coefficients, hole_size=0.0625):
    # -------------------------
    # User Parameters
    # -------------------------
    ...
    # 8x8 hole => i_hole_min = i_c - 4, i_hole_max = i_c + 3, etc.
    i_hole_min = i_c - 4
    i_hole_max = i_c + 3
    j_hole_min = j_c - 4
    j_hole_max = j_c + 3

    logging.info(f"Hole center (i_c, j_c)=({i_c}, {j_c}) "
                 f" => hole range i=[{i_hole_min},{i_hole_max}], "
                 f"j=[{j_hole_min},{j_hole_max}]")
    # blocks_ij = get_blocks_around_hole(N, i_hole_min, i_hole_max, j_hole_min, j_hole_max)
    Nx, Ny = 128, 128
    blocks_ij = [
        # Bottom-left  (i=0..59, j=0..59)
        (range(0,60),   range(0,60)),
        # Bottom-center (i=60..67, j=0..59)
        (range(60,68),  range(0,60)),
        # Bottom-right  (i=68..127, j=0..59)
        (range(68,Nx),  range(0,60)),

        # Middle-left   (i=0..59, j=60..67)
        (range(0,60),   range(60,68)),
        # Middle-right  (i=68..127, j=60..67)
        (range(68,Nx),  range(60,68)),

        # Top-left      (i=0..59, j=68..127)
        (range(0,60),   range(68,Ny)),
        # Top-center    (i=60..67, j=68..127)
        (range(60,68),  range(68,Ny)),
        # Top-right     (i=68..127, j=68..127)
        (range(68,Nx),  range(68,Ny)),
    ]

    logging.info(f"Hole indices => i=[{i_hole_min}..{i_hole_max}], j=[{j_hole_min}..{j_hole_max}]")
    logging.info(f"Number of blocks around hole: {len(blocks_ij)}")
...
def main():
    ...
    if fixed_params:
        ...
        random_coeffs = generate_random_coefficients(p)
    ...
    for i, folder in enumerate(new_folders, start=1):
        ...
        # 3A) Compute new hole coordinates based on center, size, etc.
        hole_dict = compute_hole_coords(cx, cy, hole_size=0.0625)

        # 3B) Parse & modify blockMeshDict, run blockMesh
        try:
        ...
        print(f"Design point {i}: Moved hole center to ({cx}, {cy}). BlockMesh successful.")

        # 3C) Run your field generation steps
        #     (These are presumably defined elsewhere in your code)
        if not fixed_params:
            p = 4  # Dimension for coefficients (or parameterize this as needed)
            random_coeffs = generate_random_coefficients(p)
        generate_U_file(folder, cx, cy, random_coeffs, hole_size=0.0625)         # This replaces the old generate_setfields.py call
