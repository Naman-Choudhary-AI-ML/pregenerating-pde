...
def parse_simulation(sim_folder, Umax_simulation=None, L=2, nu=1.0e-4):
    """
...
def gather_all_simulations(sim_folders, grid_shape=(128, 128), c_file_name="0/C", L=2, nu=1.0e-4):
    """
...
def update_Umax_in_simulation_folder(sim_folder, re_value, L=2, nu=1.0e-4):
    """
...
nu = 1.0e-4     # kinematic viscosity [m²/s]
...
def main(batch_name: str, total_trajectories: int):
    save_dir = f"/data/user_data/namancho/FPO_Cylinder_Multiple_Holes/{batch_name}"
    ...
    re_values = generate_normal_re_values(total_trajectories, mean=9000, std_dev=200)
