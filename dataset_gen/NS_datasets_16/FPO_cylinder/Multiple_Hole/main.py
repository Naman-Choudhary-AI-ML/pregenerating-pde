...
def parse_simulation(sim_folder, Umax_simulation=None, L=16, nu=1.53e-5):
    """
...
def gather_all_simulations(sim_folders, grid_shape=(128, 128), c_file_name="0/C", L=16, nu=1.53e-5):
    """
...
def update_Umax_in_simulation_folder(sim_folder, re_value, L=16, nu=1.53e-5):
    """
...
scaled_hole1 = (0.625 * 16, 0.9375 * 16, 0.125 * 16, 0.125 * 16)
scaled_hole2 = (1.25 * 16, 0.9375 * 16, 0.125 * 16, 0.125 * 16)
new_holes = [scaled_hole1, scaled_hole2]
def generate_blockMeshDict(domain=(0, 16, 0, 16),
                           resolution=(128, 128),
...
def randomize_holes(n, hole_size=(1.0, 1.0), domain=(0, 16, 0, 16),
                    grid_step=0.125, allow_overlap=False,
...
L  = 16.0        # characteristic length [m]
...
def main(batch_name: str, total_trajectories: int):
    save_dir = f"/data/user_data/vhsingh/FPO_Cylinder_Multiple_Holes/{batch_name}"
