...
def run_icoFoam(folder: str) -> bool:
    """
    Runs icoFoam in the specified folder, logs each line of output,
    ...
    """
    cmd = ["icoFoam"]
    logging.info(f"➤ Starting icoFoam in {folder}")
    ...
    try:
        ...
        if proc.returncode == 0:
            msg = (f"✔ icoFoam completed in {elapsed:.2f}s "
                   f"({iterations} steps, {t_per_iter:.2f}s/step)")
            print(msg)
            logging.info(msg)
            return True
        else:
            msg = (f"✘ icoFoam failed after {elapsed:.2f}s "
                   f"({iterations} steps, {t_per_iter:.2f}s/step)")
            print(msg)
            logging.warning(msg)
            return False
        ...
        err = "icoFoam executable not found in $PATH"
        print(f"ERROR: {err}")
        logging.error(err)
        return False

    except Exception as e:
        err = f"Exception running icoFoam: {e}"
        print(f"ERROR: {err}")
        logging.error(err)
        return False
...
def compute_parabolic_inlet(Umax, H=16, num_points=128):
    """
...
def parse_simulation(sim_folder, expected_n_points=16384, Umax_simulation=None, L=16, nu=1.53e-5):
    """
...
def gather_all_simulations(sim_folders, expected_n_points=16384, L=16, nu=1.53e-5):
    """
...
def main(batch_name: str, total_trajectories: int):
    save_dir = f"/data/user_data/vhsingh/test_FPO_cylinder_reg_new/{batch_name}"
        ...
        if converged_folders:
            try:
                ...
                if final_results is not None:
                    batch_file = os.path.join(save_dir, f"results_batch_{batch_index}.npy")
