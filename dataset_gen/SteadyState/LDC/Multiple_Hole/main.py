...
def run_simpleFoam(folder):
    """Runs simpleFoam for the specified folder and returns True if it converged, False otherwise."""
    command = ["simpleFoam"]
    print(f"Running simpleFoam in folder: {folder}")
    try:
        process = subprocess.Popen(command, cwd=folder)
        process.communicate()
        if process.returncode == 0:
            print(f"simpleFoam simulation completed successfully in folder: {folder}")
            logging.info(f"simpleFoam simulation completed successfully in folder: {folder}")
            return True
        else:
            print(f"simpleFoam simulation failed or did not converge in folder: {folder}")
            logging.warning(f"simpleFoam simulation failed or did not converge in folder: {folder}")
            return False
    except FileNotFoundError:
        print("Error: simpleFoam command not found.")
        logging.error("Error: simpleFoam command not found.")
        return False
    except Exception as e:
        print(f"Error running simpleFoam: {e}")
        logging.error(f"Error running simpleFoam: {e}")
        return False
...
def generate_normal_re_values(num_samples, mean=500, std_dev=400, min_re=10, max_re=1000):
    """
...
def generate_empirical_re_values(existing_re_values, num_samples):
    """
    Samples new Re values from the distribution of existing ones.
    
    Parameters:
    - existing_re_values: np.array of known Re values (1 per trajectory).
    - num_samples: how many new Re values to sample.
    
    Returns:
    - np.array of sampled Re values.
    """
    return np.random.choice(existing_re_values, size=num_samples, replace=True)
...
def main(batch_name: str, total_trajectories: int):
    save_dir = f"/data/user_data/namancho/SteadyState/LDC_Multiple_Holes_Final/{batch_name}"
    ...
    # re_values = generate_normal_re_values(total_trajectories, mean=500, std_dev=400)
    # Generate all Re values upfront
    # re_values = generate_normal_re_values(total_trajectories, mean=5000, std_dev=2000)
    Flowbench_data = np.load('/data/group_data/sage_lab_complex_geometry/FlowBench_LDC_NS_2D_128x128/LDC_NS_2D/128x128/Combined_Input_X.npz')['data']
    # print("Input array shape:", input_array.shape)
    # Extract the first channel (Re) from the data
    # Extract 1 Re value per trajectory (e.g., top-left grid cell)
    existing_re_values = Flowbench_data[:, 0, 0, 0]  # Shape: (1000,)
    re_values = generate_empirical_re_values(existing_re_values, total_trajectories)
    logging.info(f"Generated {len(re_values)} Re values for simulation.")
