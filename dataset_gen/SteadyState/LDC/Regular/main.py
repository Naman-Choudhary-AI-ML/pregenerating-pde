...
def run_simpleFoam(folder: str) -> bool:
    """
    Runs simpleFoam in the specified folder, logs each line of output,
    counts iterations (lines starting with 'Time ='), and prints+logs
    total runtime and time per iteration.
    Returns True if returncode==0, False otherwise.
    """
    cmd = ["simpleFoam"]
    logging.info(f"➤ Starting simpleFoam in {folder}")
    ...
    try:
        ...
        if iterations:
            t_per_iter = elapsed / iterations
        else:
            t_per_iter = float("nan")

        if proc.returncode == 0:
            msg = (f"✔ simpleFoam completed in {elapsed:.2f}s "
                   f"({iterations} steps, {t_per_iter:.2f}s/step)")
            print(msg)
            logging.info(msg)
            return True
        else:
            msg = (f"✘ simpleFoam failed after {elapsed:.2f}s "
                   f"({iterations} steps, {t_per_iter:.2f}s/step)")
            print(msg)
            logging.warning(msg)
            return False
        ...
        err = "simpleFoam executable not found in $PATH"
        print(f"ERROR: {err}")
        logging.error(err)
        return False

    except Exception as e:
        err = f"Exception running simpleFoam: {e}"
        print(f"ERROR: {err}")
        logging.error(err)
        return False
...
def update_U_file(sim_folder, Umax):
    ...
    pattern = r"(movingWall\s*\{\s*type\s*fixedValue;\s*value\s*uniform\s*\()\d*\.?\d*(\s0\s0\);)"
...
def main(batch_name: str, total_trajectories: int):
    save_dir = f"/data/user_data/namancho/SteadyState/LDC_Regular_Final/{batch_name}"
    ...
    while trajectories_done < total_trajectories:
        ...
        for folder in batch_folders:
            ...
            converged = run_simpleFoam(folder)
